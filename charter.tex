...
\begin{itemize}
	...
	\item Probabilidad de ocurrencia (O): 2, previo al diseño de las placas entregables a cliente se realizarán placas de prueba para validar el funcionamiento y hacer pruebas de sistema, por lo ya habrán pruebas previas que validen el diseño.
\end{itemize}   

Riesgo 2: Demoras ocasinadas por aduana durante el proceso de importación de componentes.
\begin{itemize}
	\item Severidad (S): 8, según sea el monto de lo que se importe y disposiciones internas de aduana, el pedido puede permanecer retenido un tiempo entre semanas o hasta incluso meses y esto afectaría significativamente al proyecto.
	\item Probabilidad de ocurrencia (O): 6, en el contexto macroeconómico actual se espera que puedan adoptarse medidas que restrinjan la importación de componentes electrónicos y afines.
\end{itemize}

Riesgo 3: Destrucción o extravios de prototipos.
\begin{itemize}
	\item Severidad (S): 7, esto puede afectar los tiempos de desarrollo con demoras significativas si esto ocurriese, por ejemplo, durante las pruebas de sistema.
...
\begin{itemize}
	\item Severidad (S): 7, esto podría aumentar el tiempo de diseño de sistema, a nivel de que si no se desarrolla correctamente afectaría en gran medida a la integridad y pérdida de los datos transmitidos.
	\item Probabilidad de ocurrencia (O): 3, siendo que se espera una baja tasa de datos generada por los nodos sensores, no existirá gran concurrencia de datos y por ende no hay necesidad de implementar algoritmos ni protocolos de gran complejidad.
\end{itemize}
...
\begin{itemize}
	...
	\item Probabilidad de ocurrencia (O): 6, considerando que para disminuir tiempo de desarrollo se utilizarán módulos comerciales, es posible que el modelo elegido tenga errores de diseño que contribuyan negativamente al establecimiento del enlace de radio.
\end{itemize}
...
Riesgo 2: Para esto se buscará minimizar la cantidad de componentes electrónicos importados desde el exterior, y en caso de ser necesario, tener mayor discresión en el volumen de compra para no ser retenido por aduana.
\begin{itemize}
...
Riesgo 6: Siendo que el enlace de radio es uno de los elementos más críticos del proyecto, se le asignará una mayor prioridad temporal al estudio de alternativas y a la validación del enlace, para así confirmar el transceiver utilizado. De esta forma se minimiza el proceso de reingeniería en caso de ser necesario un cambio sobre dicho aspecto.

\begin{itemize}
\item Severidad (S*): 6, la severidad se verá disminuida en el sentido de que al ocurrir más temprano en el tiempo se tiene mayor margen de maniobra.
\item Probabilidad de ocurrencia (O*): 3, ya que habiendo realizado un análisis detallado es menos probable que la alternativa elegida presente fallas en el enlace de radio.
\end{itemize}
...
\begin{itemize}
	...
	\item Validación: Se le solicitará al cliente que valide el requerimiento mediante inspección visual.
\end{itemize}
...
\begin{itemize}
	\item Verificación: Se eliminarán todas las vías de comunicación posibles, a excepción de internet, entre el nodo gateway la computadora utilizada para la recepción de paquetes.
...
\begin{itemize}
	\item Verificación: Se forzará cada uno de los errores por software y se verificará que el encendido de los indicadores se corresponda con una lista de errores predefinidos.
...
\begin{itemize}
	\item Verificación: Se vericará la cantidad de ciclos de carga con la hoja de datos de fabricante.
...
\begin{itemize}
	\item Verificación: Se verificará el correcto grabado en memoria de datos conocidos previamente.
	\item Validación: Se realizará una adquisición de mediciones a lo largo de un tiempo predeterminado y presentarán los archivos frente al cliente.
\end{itemize}
...
\begin{itemize}
	...
	\item Validación: Se le indicará a cliente el campo de estampa de tiempo en los paquetes recibidos por internet.
\end{itemize}
